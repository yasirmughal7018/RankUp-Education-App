...
| --- | --- | --- | --- |
| **Mobile** | [`Mobile/`](Mobile/) | Flutter + Riverpod | Student, Parent, Teacher, Coordinator, and admin mobile experience |
...
API-backed roles: `PortalAdmin`, `SchoolAdmin`, `CampusAdmin`, `Teacher`, `Coordinator`, `Student`, `Parent`.

- Roles are stored in `app_user_roles` (not on `app_users`). **Student** and **PortalAdmin** are exclusive; SchoolAdmin / CampusAdmin / Teacher / Parent / Coordinator may combine (common case: **Parent + Teacher**). Former Tutor accounts are remapped to Parent.
