...
interface BulkRejectRoleRequestsDialogProps {
  open: boolean;
  count: number;
...
export function BulkRejectRoleRequestsDialog({
  open,
  count,
  isSubmitting,
  onClose,
  onConfirm,
}: BulkRejectRoleRequestsDialogProps) {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Reject role requests</DialogTitle>
          <DialogDescription>
            Reject {count} role request{count === 1 ? "" : "s"}? They can submit
        ...
        <div className="space-y-2">
          <label
            htmlFor="bulk-role-request-reject-reason"
            className="text-sm font-medium text-foreground"
          >
            Rejection reason <span className="text-destructive">*</span>
          </label>
          <textarea
            id="bulk-role-request-reject-reason"
            rows={4}
            value={reason}
            disabled={isSubmitting}
            onChange={(event) => setReason(event.target.value)}
            maxLength={1000}
            placeholder="Explain why these role requests are being rejected (min 10 characters)…"
