  ...
  sections: [
    {
      children: [
        ...
        bullet("Who can request: Teacher, Student, Parent (school+campus); CampusAdmin (campus only)."),
        bullet("PortalAdmin / SchoolAdmin cannot request via profile."),
        bullet("Web Profile: separate School/campus section + Request button (not Save profile)."),
        bullet("Confirmation popup required before submit."),
        bullet("On confirm: create pending request, set is_active=false, revoke sessions, logout → /account-locked."),
        bullet("Apply (unlock + move): SchoolAdmin OR PortalAdmin."),
        bullet("CampusAdmin soft-approve only (account stays locked)."),
        ...
        bullet("QA-34: CampusAdmin soft-approve does not unlock"),
