  ...
  sections: [
    {
      children: [
        ...
        p("Version: current codebase · Date: 16 Aug 2026 (Tutor role)"),
        p(
          "Aligned with UserRoleRules (Teacher+Parent+Coordinator+Tutor combinable; SchoolAdmin/CampusAdmin exclusive), PortalAdmin-only Tutors directory, email-username, scoped activation, forgot-password first-wins. Full scenarios: docs/03_RankUp_User_Creation_Approval_QA.html",
        ),
        ...
        bullet("Username = email (required) for Student / Parent / Teacher / Tutor self-request and directory create (incl. SchoolAdmin / CampusAdmin)."),
        bullet("Mobile optional. Student roll required only when a school is selected. Parent/Tutor: no school/campus/roll."),
        bullet("Login lookup: username (email) → cnic → mobile."),
        bullet("Queue: no school / Parent / Tutor → PortalAdmin; school only → SchoolAdmin + PortalAdmin; school+campus → CampusAdmin + SchoolAdmin + PortalAdmin."),
        bullet("Activation: PortalAdmin any; SchoolAdmin Student/Teacher in school; CampusAdmin Student/Teacher in campus; no-school/Parent/Tutor → PortalAdmin only."),
        bullet("Soft-reject keeps row (rejected_at); same email can re-request after reject."),

        h2("3. Multi-role & directory companions"),
        bullet("Combinable: Teacher + Parent + Coordinator + Tutor. Exclusive: Student, PortalAdmin, SchoolAdmin, CampusAdmin."),
        bullet("List ⋯ Add role (no companion checkboxes on Create/Edit). Teacher→Coordinator: code only; school/campus read-only."),
        bullet("SchoolAdmin/CampusAdmin must NOT Add/Remove Parent or Tutor on Teachers or Coordinators (PortalAdmin only)."),
        bullet("Tutors directory (/admin/directory/tutors): PortalAdmin only — create, activate/deactivate, link/unlink students."),
        bullet("Remove companion only; never remove list primary role or sole remaining role; Parent/Tutor remove blocked while linked students remain."),

        h2("4. School / campus change"),
        bullet("Who can request: Teacher, Student, CampusAdmin (campus only), Coordinator. Parent and Tutor cannot request."),
        ...
        bullet("PortalAdmin creates SchoolAdmin, CampusAdmin, and Tutors; SchoolAdmin creates CampusAdmin (own school)."),
        bullet("Coordinators create requires coordinator code."),
        bullet("Tutor–student links: by CNIC or username (PortalAdmin)."),
        ...
        bullet("Notify: Student → School+Campus+Parent+Portal; Teacher → School+Campus+Portal; CampusAdmin → School+Portal; SchoolAdmin/Parent/Tutor → Portal."),
        ...
        bullet("/admin/directory/teachers|parents|coordinators|tutors — companion grant/remove (Tutors PortalAdmin)"),
        ...
        bullet("POST /api/auth/register — email username (Student/Parent/Teacher/Tutor)"),
        ...
        bullet("POST/DELETE /api/directory/tutors/{id}/students — PortalAdmin tutor–student links"),
        ...
        bullet("QA-01..05: scoped activation (School/Campus/Portal); Parent/Tutor PortalAdmin-only"),
        ...
        bullet("QA-40: multi-role school change follows active role (Teacher vs Parent/Tutor)"),
        bullet("Directory grant Tutor; PortalAdmin-only Parent/Tutor companions; Tutors directory parity"),
