  ...
  sections: [
    {
      ...
      children: [
        ...
        rich([
          ...
          {
            text: "app_user_roles + app_user_approval · soft-reject  ·  Date: 14 Jul 2026",
            color: MUTED,
            size: 18,
          },
        ]),
        ...
        table(
          ...
          [
            [
              "Pending approval",
              "is_active=false AND password_hash NULL AND rejected_at NULL",
              "PendingApproval",
              "No",
            ],
            [
              "Approved, needs password",
              "is_active=true AND password_hash NULL (must_change_password=true)",
              "NeedsPasswordSetup",
            ...
            [
              "Deactivated",
              "is_active=false with password set (directory deactivate)",
              "throws “not active” (not a status enum)",
              "No",
            ],
            [
              "Rejected (soft)",
              "Row kept; rejected_at set; rejector app_user_approval.is_approved=false",
              "Rejected",
              "No — may submit a new request",
            ],
          ],
          [1800, 3200, 2200, 2160],
        ),
        ...
        bullet(
          "Reject is a soft-reject (no hard delete of app_users). Unique indexes on username/CNIC ignore rows where rejected_at IS NOT NULL.",
        ),
        h3("Role lookup IDs"),
        table(
          ["Role", "Lookup ID"],
        ...
        table(
          ...
          [
            ...
            [
              "Reject registrations (soft)",
              "All pending",
              "Own school (UI hides after they approve)",
              "Own campus (UI hides after they approve)",
              "—",
              "—",
              "—",
            ],
        ...
        bullet("Username on request: CNIC if provided, otherwise mobile. Must be unique among non-rejected rows (rejected_at IS NULL)."),
        ...
        bullet("Approve / Reject buttons are hidden for that admin; UI shows: Approved — awaiting Portal Admin. (API flag: currentUserHasApproved)."),
        bullet(
          "Pending with lists remaining reviewers where is_approved IS NULL and approved_at IS NULL. After CampusAdmin approval (is_approved=true), pending SchoolAdmin rows are omitted from Pending with (SchoolAdmin DB row may still exist as pending).",
        ),
        ...
        h3("Soft reject (critical — not a hard delete)"),
        table(
          ["What", "Behaviour"],
          [
            [
              "app_users",
              "Row kept. Sets rejected_at; keeps is_active=false, password_hash NULL. Does NOT delete the user.",
            ],
            [
              "Rejector’s app_user_approval",
              "is_approved=false, approved_at set (RecordRejected). If no queue row existed, one is created then marked rejected.",
            ],
            [
              "Other admins’ pending rows",
              "Left as-is (is_approved still NULL). They fall off the pending list because the user is rejected.",
            ],
            ["Pending list", "Rejected users excluded (rejected_at IS NULL filter)."],
            [
              "login-status",
              "Rejected — Your registration request was rejected. You may submit a new request.",
            ],
            [
              "Re-request",
              "Allowed. Uniqueness ignores rejected rows. Same CNIC/mobile creates a new app_users + new approval queue. Login lookup prefers the non-rejected row.",
            ],
            [
              "app_user_approval.is_approved",
        ...
        numbered(2, "Branch: PendingApproval | Rejected | NeedsPasswordSetup | Ready."),
        numbered(3, "For Ready only: Sign in → POST /api/auth/login."),
        p(
          "Login identifier resolution: username → CNIC → mobile (prefers non-rejected row when a soft-rejected duplicate exists).",
        ),
        p(
          "While SchoolAdmin / CampusAdmin have approved but PortalAdmin has not, login-status remains PendingApproval.",
        ),
        p("PendingApproval and Rejected keep the user on the identifier step (info message; no password step)."),
        ...
        table(
          ...
          [
            [
              "Login status UI",
              "Enter identifier; see Pending / Rejected / Needs password / Ready / not active",
              "Web + Mobile",
            ],
            [
              "Pending list",
              "Appears while is_active=false, no password, and rejected_at NULL (includes post–Campus/School approval)",
              "Web + Mobile",
            ],
            ["Directory list", "Active / Inactive column and filters", "Web only"],
            [
              "Database",
              "app_users.is_active, password_hash, rejected_at, app_user_roles, app_user_approval.is_approved",
              "QA / DB",
            ],
        ...
        ...scenario("QA-10", "Reject registration (soft-reject)", "Web + Mobile + API / DB", [
          "Create a pending request (optionally with school+campus so multiple app_user_approval rows exist).",
          "Admin Rejects from Approvals (confirm dialog → success: “{name} was rejected.”).",
          "Confirm removed from Approvals for all admins.",
          "DB: app_users.rejected_at is set; user row still exists (not deleted).",
          "DB: rejector’s app_user_approval has is_approved=false; other admins’ pending rows may still be is_approved NULL.",
          "Login with same identifier → status Rejected (“…You may submit a new request.”).",
          "Submit Request Access again with same CNIC/mobile → succeeds (new app_users + new approval queue).",
          "Login with same identifier after re-request → PendingApproval (new row preferred over soft-rejected one).",
        ], "Soft-reject only; trail kept; login-status Rejected then re-request → PendingApproval on new row.", "Do not expect “No account found” after reject — that was the old hard-delete behaviour."),
        ...
        table(
          ...
          [
            ...
            ["12", "Reject → soft-reject (row kept) + login-status Rejected", "☐", "☐", ""],
            ["13", "After reject: rejected_at set; rejector is_approved=false", "DB", "—", ""],
            ["14", "After reject: same CNIC/mobile can re-request (new row)", "☐", "☐", ""],
            ["15", "Directory create → NeedsPasswordSetup (no approval queue)", "☐", "login", ""],
            ["16", "PortalAdmin creates SchoolAdmin; first login set password", "☐", "☐", ""],
            ["17", "Portal/School Admin creates CampusAdmin; first login set password", "☐", "☐", ""],
            ["18", "CampusAdmin campus-scoped list + directory create", "☐", "☐", ""],
            ["19", "Multi-role add Teacher to Parent; switch role", "☐", "☐", ""],
            ["20", "Student / PortalAdmin cannot combine roles", "☐", "—", ""],
            ["21", "Deactivate → not active", "☐", "☐", ""],
            ["22", "Required * dark red bold", "☐", "☐", ""],
            ["23", "Approvals UI: Pending with, no Admin Target, Read more", "☐", "—", ""],
            ["24", "Switch role survives token refresh", "☐", "☐", ""],
            ["25", "No role/admin_target on app_users; soft-reject columns present", "DB", "—", ""],
          ],
        ...
        table(
          ...
          [
            ...
            [
              ...
              "Portal/School/Campus Admin",
              "Soft-reject: rejected_at + is_approved=false (no delete)",
            ],
            [
              "POST",
              "/api/auth/login-status",
              "Anonymous",
              "PendingApproval | Rejected | NeedsPasswordSetup | Ready",
            ],
