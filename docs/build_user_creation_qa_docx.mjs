  ...
  sections: [
    {
      children: [
        ...
        p("Version: current codebase · Date: 23 Aug 2026"),
        p(
          "Aligned with UserRoleRules (Teacher+Parent+Coordinator combinable; SchoolAdmin/CampusAdmin exclusive), email-username, scoped activation, forgot-password first-wins. Former Tutor accounts are remapped to Parent. Full scenarios: docs/03_RankUp_User_Creation_Approval_QA.html",
        ),
        ...
        bullet("Mobile optional. Student roll required only when a school is selected. Parent: no school/campus/roll."),
        ...
        bullet("SchoolAdmin/CampusAdmin must NOT Add/Remove Parent on Teachers or Coordinators (PortalAdmin only)."),
        bullet("Remove companion only; never remove list primary role or sole remaining role; Parent remove blocked while linked students remain."),

        h2("4. School / campus change"),
        bullet("Who can request: Teacher, Student, CampusAdmin (campus only), Coordinator. Parent cannot request."),
        ...
        bullet("Parent–student links: by CNIC or username (directory or parent self-link)."),
        ...
        bullet("Notify: Student → School+Campus+Parent+Portal; Teacher → School+Campus+Portal; CampusAdmin → School+Portal; SchoolAdmin/Parent → Portal."),
        ...
        bullet("POST /api/auth/register — email username (Student/Parent/Teacher)"),
        ...
        bullet("POST/DELETE /api/directory/parents/{id}/students — parent–student links"),
        ...
        bullet("QA-01..05: scoped activation (School/Campus/Portal); Parent PortalAdmin-only"),
        ...
        bullet("Directory grant Parent companions; Parent–student links"),
