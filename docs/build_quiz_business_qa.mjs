...
const lifecycleStatuses = [
  ...
  ["61", "Published", "PortalAdmin publish step only. Set when lifecycle moves Draft → Published after approval gates are satisfied. Shared catalog: Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent may list and open the quiz regardless of creator or school. Those roles may also assign it to their own students/children. Students receive it via assignment or AudienceScope=Public only — publish does not auto-enroll every student. Assigning students does not change this status; it stays Published. A control on the quiz opens the assigned children/students list."],
  ["62", "Assigned (legacy)", "Retired quiz-row status. Not written anymore. Existing rows remap to Published. Per-student assignment lives on quiz_assignment only."],
  ["63", "Archived", "Soft-retired when assignments exist (IsActive=false). Only PortalAdmin may archive a Published or Assigned quiz (including any student/child assignment). Same shared staff-catalog visibility as Published. Hard-deleted instead when there are no assignments."],
];
...
const approvalStatuses = [
  ...
  ["41", "SchoolApproved", "SchoolAdmin or CampusAdmin approved a Teacher/Coordinator quiz, or SchoolAdmin approved a CampusAdmin-created quiz (Pending → SchoolApproved). Does not apply to SchoolAdmin/Parent created quizzes (those skip this step). Does not publish — lifecycle stays Draft. Owner cannot edit in place; they send an edit request. Not assignable until PortalAdmin publishes. PortalAdmin queue may include SchoolApproved rows."],
  ["42", "Approved", "PortalAdmin final approval. Teacher/Coordinator/CampusAdmin: from Pending or SchoolApproved. SchoolAdmin/Parent created: from Pending directly. Does not by itself publish — lifecycle stays Draft until PortalAdmin runs publish. Owner cannot edit in place; they send an edit request. school-type: portal approve + publish may occur together via publish endpoint."],
  ["43", "Rejected", "Denied by SchoolAdmin, CampusAdmin, or PortalAdmin (in scope). Rejection reason required. Lifecycle stays Draft. Only the quiz creator may Resubmit for approval (Pending). The rejecting PortalAdmin (or any non-owner) does not see or use Resubmit. Cannot be approved until the creator resubmits."],
];
...
const approvalVsPublishRules = [
  ...
  "CampusAdmin created quiz: own SchoolAdmin may school-approve (Pending → SchoolApproved). Other CampusAdmins cannot endorse it. PortalAdmin then approves (→ Approved) and publishes. SchoolAdmin-created quizzes stay PortalAdmin only (Pending → Approved), same as Parent school-type.",
  ...
  "Other authorized creators: CampusAdmin creates school quiz types and submits to SchoolAdmin (Pending → SchoolApproved), then PortalAdmin. SchoolAdmin creates school quiz types and submits to PortalAdmin only (Pending → Approved, then PortalAdmin publishes). school-type: Parent create; PortalAdmin approve + publish before assign.",
  "Preserve published catalog visibility: Published school-type quizzes are a shared catalog for Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent (any school, any creator) — not CreatedBy or school scope (see §6a). Students are excluded from that catalog.",
];
...
const resultStatuses = [
  ...
  ["21", "Completed", "Written when the owner marks the attempt Completed (finalize-review / IsReviewDone). Student UI shows Completed only after that action."],
  ["22", "Under Review", "Written on submit while IsReviewDone=false. Student UI overlays Results pending (window still open) or Partial results (due date passed, auto-graded announced) until the owner clicks Completed."],
...
const quizAssignmentFields = [
  ...
  ["AssignedById", "Teacher, Coordinator, or Parent who created the assignment. Students do not assign quizzes in the current model."],
  ...
  ["AllowedAttempts", "Attempt quota for this assignment. Always 1. Quiz definition has no Allowed Attempts field. Retry and reassign do not increase it."],
  ...
  ["IsReviewDone", "True after teacher/parent finalize-review for this student’s subjective answers."],
  ["CreatedDate", "When the assignment row was created."],
  ["ModifiedDate", "Updated on review finalize or result-status change."],
];
...
const quizTypes = [
  ["1", "Practice", "Teacher / Coordinator / SchoolAdmin / CampusAdmin / PortalAdmin / Parent", "Learning-oriented school quiz. Admin approval before assign. One allowed attempt (same as other types). Optional time; answers may show after submit (type-specific UX still soft)."],
...
const lifecycle = [
  ["Create (any authorized role)", "Draft + Pending", "Approval always starts Pending. Lifecycle Draft. Parent and school staff create school types (Practice, Assessment, Competition, Surprise). CampusAdmin stamps school+campus from the auth token. Not assignable."],
  ...
  ["Submit for approval (Teacher / Coordinator / SchoolAdmin / CampusAdmin / Parent)", "Draft + Pending", "Owner action only (publish endpoint for non–PortalAdmin). PortalAdmin never submits or resubmits another author’s quiz. Lifecycle stays Draft. ≥1 question. Teacher/Coordinator notify school+campus+portal. CampusAdmin notify SchoolAdmin + PortalAdmin. SchoolAdmin/Parent notify PortalAdmin only. Not assignable."],
  ["Approve (SchoolAdmin)", "Draft + SchoolApproved", "Teacher/Coordinator/CampusAdmin quizzes in own school. Pending → SchoolApproved. Cannot school-approve another SchoolAdmin quiz. Does not publish. Creator cannot self-approve."],
  ["Approve (CampusAdmin)", "Draft + SchoolApproved", "Teacher/Coordinator quizzes only. Pending → SchoolApproved in own campus. Cannot school-approve SchoolAdmin/CampusAdmin/Parent quizzes. Does not publish. Creator cannot self-approve."],
  ...
  ["Publish (PortalAdmin only)", "Published + Approved", "Draft → Published. Teacher/Coordinator quiz: requires Approved or SchoolApproved while Draft. SchoolAdmin/CampusAdmin/Parent created (and school-type): portal may approve from Pending, or approve+publish in one publish action. Only then assignable."],
  ["Publish (SchoolAdmin / CampusAdmin / Teacher / Coordinator / Parent)", "—", "Not allowed. Approve (where permitted) or submit for approval only; PortalAdmin publishes."],
  ["Reject (SchoolAdmin / CampusAdmin / PortalAdmin)", "Draft + Rejected", "In-scope only. Reason required. SchoolAdmin: Pending Teacher/Coordinator/CampusAdmin quizzes in school. CampusAdmin: Pending Teacher/Coordinator in campus. Portal: Pending (including SchoolAdmin-created), SchoolApproved, or school-type queue. Creator must resubmit."],
  ["Assign", "Published", "Requires Lifecycle Published (legacy Assigned rows still assignable). Not Draft. Teacher quizzes: Approval=Approved. school-type: Published + Approved; assign to linked children. ≥1 question. Does not change quiz lifecycle."],
  ["Cancel upcoming", "Published", "Deletes only assignments with StartDateTime > now. Quiz stays Published. Never uses Cancelled or Assigned lifecycle."],
  ["Archive / Delete", "Deleted or Archived", "Published or Assigned (including assigned to students/children): PortalAdmin only. Other roles cannot archive after publish or after any assignment exists. Owners may hard-delete their own Draft (no assignments). school-type delete/archive → PortalAdmin only. No assignments → hard delete; else soft Archive."],
  ["Unarchive", "Published or Assigned", "PortalAdmin only (same as archive of published/assigned). Restores Assigned if any assignments exist, else Published; IsActive=true."],
...
const quizEditRequestRouting = [
  ...
  ["SchoolAdmin / CampusAdmin / Parent", "PortalAdmin only", "PortalAdmin must approve"],
...
const quizEditRequestRules = [
  ...
  "SchoolAdmin, CampusAdmin, and Parent requests go to PortalAdmin only.",
...
const permissions = [
  ["Create / duplicate own quizzes", "Yes", "Yes (own, school types)", "Yes (own, school types)", "Own", "Own", "Own (school types)", "No"],
  ...
  ["Submit / resubmit for approval (owner)", "No — reviews only; does not submit or resubmit another author’s quiz", "Yes (own)", "Yes (own)", "Yes (own)", "Yes (own)", "Yes (own)", "No"],
  ...
  ["Approve SchoolAdmin-created quizzes", "Yes (Pending → Approved)", "No (PortalAdmin only)", "No", "No", "No", "No", "No"],
  ["Approve CampusAdmin-created quizzes", "Yes (Pending or SchoolApproved → Approved)", "Yes (Pending → SchoolApproved, own school)", "No (no self-approve)", "No", "No", "No", "No"],
  ...
  ["Delete / archive / unarchive", "Yes (all). Only role that may archive Published or Assigned", "Delete own Draft only — cannot archive Published/Assigned", "Delete own Draft only — cannot archive Published/Assigned", "Delete own Draft only — cannot archive Published/Assigned", "Delete own Draft only — cannot archive Published/Assigned", "Delete own Draft only — cannot archive Published/Assigned", "No"],
  ["Assign / cancel / monitor / review", "Yes", "School-scoped assign (own school quizzes)", "Campus-scoped assign (own campus quizzes)*", "Shared catalog → own roster (Published school-type)", "Shared catalog → own roster (Published school-type)", "Shared catalog → linked children", "No"],
  ["Approve / reject Parent-created quizzes", "Yes (only)", "No", "No", "No", "No", "No (Parent creators)", "No"],
  ["Review submitted drafts (Draft tile → /quizzes/:id)", "Yes (Teacher/Coordinator/CampusAdmin Pending + SchoolApproved + SchoolAdmin/school-type Pending)", "Teacher/Coordinator/CampusAdmin submitted in school", "Teacher/Coordinator submitted in campus", "No", "No", "No", "No"],
  ["List / view Draft (unpublished) quizzes", "Own WIP; others only after submit for approval", "Own drafts + Teacher/Coordinator submitted in school", "Own drafts + Teacher/Coordinator submitted in campus", "Own", "Own", "Own (Parent)", "No"],
  ["List / view Published quizzes (manage catalog)", "All school-type (any school/creator)", "All school-type (any school/creator)", "All school-type (any school/creator)", "All school-type ∪ roster assignments", "All school-type ∪ roster assignments", "All school-type ∪ linked-child assignments ∪ own", "Assigned ∪ student Public audience"],
  ["Take attempts / save draft / submit", "No", "No", "No", "No", "No", "No", "Yes"],
  ["View attempt result", "Yes (any student)", "School students", "Campus students", "Roster students", "Roster students", "Linked children", "Own"],
...
const coordinatorRoleSummary = [
  ...
  "Create: Practice, Assessment, Competition, Surprise (school quiz types). Starts Draft + Pending Approval. Parent creates the same school types.",
  ...
  "Assign (Published only): may assign any published school-type quiz from the shared catalog to students in the caller’s roster (one, selected, group, allInGrade, allInSection). Not school-wide, multi-school, or public catalog.",
  "Monitor / review: roster students on viewable (catalog or owned) quizzes. Cancel-upcoming still requires quiz ownership.",
  "Catalog list: all published school-type quizzes (any school, any creator) plus own drafts — same 9-quiz example as SchoolAdmin. Optional Mine only filter on web.",
  "Cannot access the quiz edit-request queue (SchoolAdmin, CampusAdmin, PortalAdmin only).",
  "Delete: own Draft only. Cannot archive Published or Assigned quizzes — PortalAdmin only.",
];
...
const assignmentModes = [
  ...
  ["allLinked", "Parent", "Audience: all linked children."],
];

const quizListVisibilityRules = [
  "Published / Assigned / Archived school-type quizzes (Practice, Assessment, Competition, Surprise) are a shared catalog: any Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, or Parent may list and open them, regardless of who created them and which school they belong to.",
  "Example: AES SchoolAdmin creates 7 published quizzes and PortalAdmin creates 2. A Teacher, Coordinator, Parent, or ISL SchoolAdmin (who created none) still sees all 9. Creator is metadata, not a visibility filter.",
  "Parent-created published school-type quizzes are in the same shared catalog as teacher-created quizzes.",
  "Students do not see every published quiz. Students see (1) quizzes assigned to them and (2) AudienceScope=Public within the audience window. Publishing to the staff/parent catalog is not the same as the student Public audience.",
  "PortalAdmin → all published/assigned/archived quizzes, including school-type. Drafts: own WIP, plus other authors only after Submit for approval (Pending submitted, SchoolApproved, Approved, Rejected).",
  "SchoolAdmin / CampusAdmin / Teacher / Coordinator / Parent → shared published school-type catalog (all schools) plus own Draft rows. SchoolAdmin also sees Teacher/Coordinator/CampusAdmin quizzes submitted for approval in the same school; CampusAdmin sees Teacher/Coordinator submitted quizzes in the same campus. They open /quizzes/:id and use School approve / Reject. Parent also see quizzes assigned to linked children/students.",
  "Teacher / Coordinator may assign a published school-type quiz from the shared catalog to students in their roster. Parent may assign those quizzes to linked children/students. SchoolAdmin/CampusAdmin assign stays school/campus scoped. CampusAdmin/SchoolAdmin one/selected assign: Search students field has a leading search icon; results and a Selected students panel show name, username, roll, grade/section (and campus when known). Grade filter optional, not locked to quiz class. All-in-grade / all-in-section: a single required Grade control (Audience-filters Grade is hidden in those modes so it is not duplicated). CampusAdmin Assign UI omits School and Campus fields (locked to session). Assign dialog uses the shared Dialog shell (header / scroll body / sticky footer). Archive of Published/Assigned remains PortalAdmin only.",
  "Student → assignment rows for self plus Public catalog within audience window.",
  "Draft (unpublished): owner-only until the owner submits for approval. After a Teacher/Coordinator submits, PortalAdmin and the school/campus queue may see it. After a CampusAdmin submits, SchoolAdmin (same school) and PortalAdmin may review it. After a SchoolAdmin submits, only PortalAdmin reviews it. Not assignable; not in student catalogs.",
  "Manage detail (GET /manage): Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent may open Published/Assigned/Archived school-type quizzes. Draft: owner always. After Submit for approval, PortalAdmin may open any pipeline draft; SchoolAdmin may open Teacher/Coordinator/CampusAdmin pipeline drafts in the same school; CampusAdmin may open Teacher/Coordinator pipeline drafts in the same campus. Unsubmitted WIP stays owner-only.",
  "Edit settings / questions: PortalAdmin may edit any in place. Owner may edit only while Draft + Pending/Rejected, or with an unused approved edit grant (see §5b). School/campus admins may monitor/assign in their org but not edit another author's quiz.",
  "Web /quizzes list: Portal/School/Campus Admin, Teacher, Coordinator, and Parent see the shared published catalog by default; Mine only is an optional client filter. SchoolAdmin also sees in-scope Teacher/Coordinator/CampusAdmin submitted drafts under the Draft tile; CampusAdmin sees Teacher/Coordinator submitted drafts. Approve or reject on /quizzes/:id. There is no separate approval page or tile.",
  "Portal-created published school-type quizzes appear in the shared catalog even when schoolId is null.",
];
...
const studentAttemptFlow = [
  ...
  ["15", "Student sees the permitted review screen", "Covered", "Auto-graded questions announce when the assignment due date ends (later of submit and EndDateTime). Teacher-review questions and full Completed status wait until the owner clicks Completed. List/detail/result show resultAnnouncedPercent."],
...
const timeManagementAppBehaviors = [
  ...
  ["Prevent reopening after expiry unless allowed", "Covered", "Assignment window expiry blocks/expires attempts. There is no Allow Retry and no reassign of the same quiz. One attempt only."],
];
...
const attemptRules = [
  ...
  "New start blocked when ExistingAttemptCount ≥ AllowedAttempts (always 1).",
  "Assign skips students who already have a QuizAssignment row. Cancel of future unused rows deletes that row so the student can be assigned later. Historical QuizAttempt rows stay.",
...
const partialCreditExamples = [
  ...
  ["Decimal floor", "Max 3; 2 expected correct; student selects 1 correct and 1 wrong", "1.5 → award 1 (never 2 or 3)"],
...
const reviewRules = [
  ...
  "Mark answers: awarded marks in [0, MaxMarks]. After owner Completed (IsReviewDone), mark/feedback updates are rejected and the review workspace is read-only.",
...
const apiMap = [
  ["GET /api/quizzes", "Role-scoped list. Student: assigned ∪ Public audience. Teacher/Coordinator/SchoolAdmin/CampusAdmin/Parent: all Published/Assigned/Archived school-type quizzes (any school, any creator) ∪ own drafts. Teacher/Coordinator also merge quizzes assigned to roster students. SchoolAdmin also includes Teacher/Coordinator/CampusAdmin submitted pipeline drafts in the same school; CampusAdmin includes Teacher/Coordinator submitted drafts in the same campus. Parent also merge quizzes assigned to linked children. PortalAdmin: all published/assigned/archived plus pipeline drafts. Unsubmitted WIP drafts: owner only."],
  ["POST /api/quizzes", "Create Draft + Pending. Parent and school staff: school types (Practice, Assessment, Competition, Surprise). CampusAdmin school+campus from auth token. SchoolAdmin school from token (campus optional). PortalAdmin school/campus optional."],
  ...
  ["GET /api/quizzes/{id}/manage", "Manage view with questions. Published/Assigned/Archived school-type quizzes: any Teacher/Coordinator/SchoolAdmin/CampusAdmin/PortalAdmin/Parent. Draft: owner always. After Submit for approval: PortalAdmin any pipeline draft; SchoolAdmin Teacher/Coordinator/CampusAdmin pipeline drafts in the same school; CampusAdmin Teacher/Coordinator pipeline drafts in the same campus. school-type drafts: owner + PortalAdmin. Unsubmitted WIP hidden from non-owner."],
  ...
  ["POST /api/quizzes/{id}/assign", "Requires Lifecycle Published (legacy Assigned still allowed). Not Draft. Pending Approval blocked. Teacher quizzes need Approval=Approved. Creates assignments for students who do not already have a row; existing students are skipped. Lifecycle stays Published. Parent mode=group uses a parent-owned group id."],
...
const scenarios = [
  ...
  [
    "QZ-03",
    "Parent school-type → portal approve and publish",
    "Parent creates and submits a school-type quiz; PortalAdmin approves and publishes before assign.",
    "Lifecycle Draft until portal publish → Published. Approval Pending→Approved. Parent assigns only to linked children. Not assignable until Published.",
  ],
  [
    "QZ-04",
  ...
  [
    ...
    "Start blocked. There is no Allow Retry and no reassign of the same quiz.",
  ],
  [
    "QZ-09",
    ...
    "Until the due date (later of submit and EndDateTime), announced percent is 0 and all results stay pending. After the due date, auto-graded questions announce (Partial results + announced percent); teacher-review questions stay ResultPending until the owner clicks Completed. List/detail hide the student’s score percent until that owner action.",
  ],
  [
    "QZ-10",
  ...
  [
    "QZ-12",
    "Archive vs delete — PortalAdmin only after publish",
    "Teacher or SchoolAdmin tries to archive a Published quiz, or a quiz already assigned to students/children. PortalAdmin archives a Published quiz with assignments; owner deletes their own Draft with no assignments.",
    "Non–PortalAdmin archive of Published or Assigned is rejected. PortalAdmin archive with assignments → soft Archived; with no assignments → hard delete. Draft must be deleted (not archived). Delete of own Draft succeeds.",
  ],
  [
    "QZ-13",
    "Duplicate",
  ...
  [
    ...
    "Teacher quiz is SchoolApproved (or Published). Teacher sends an edit request; SchoolAdmin or CampusAdmin or PortalAdmin approves; Teacher edits; then resubmits. SchoolAdmin, CampusAdmin, or Parent owner sends the request to PortalAdmin only.",
  ...
  [
    ...
    "SchoolAdmin creates a Practice quiz, adds questions, submits for approval. Status shows Approval Pending. PortalAdmin clicks Approve. Another SchoolAdmin opens the Draft tile.",
    "PortalAdmin Approve succeeds: Pending → Approved (lifecycle stays Draft). No SchoolApproved step. The other SchoolAdmin does not see this quiz as reviewable and cannot school-approve it. PortalAdmin then publishes separately (Draft → Published).",
  ],
  [
    "QZ-27",
    "SchoolAdmin school-approves a CampusAdmin-created quiz",
    "CampusAdmin creates a Practice quiz, adds questions, submits for approval. SchoolAdmin of the same school opens /quizzes Draft tile and the quiz.",
    "SchoolAdmin sees School approve / Reject. School approve → SchoolApproved (Draft). CampusAdmin cannot approve their own quiz. PortalAdmin then Approves and Publishes.",
  ],
  [
    "QZ-16",
  ...
  [
    "QZ-17",
    "school-type assign scope (Parent)",
    "Parent tries to assign school-type quiz to an unrelated student or whole school.",
    "Rejected. Parent → linked children only.",
  ],
  [
    "QZ-18",
  ...
  [
    "QZ-22",
    "Published school quizzes are a shared catalog",
    "PortalAdmin publishes Quiz A and B (any school, including null schoolId). AES SchoolAdmin publishes Quiz C. A Teacher, Coordinator, Parent, and ISL SchoolAdmin (who created none) each open /quizzes.",
    "Teacher, Coordinator, Parent, PortalAdmin, AES SchoolAdmin, and ISL SchoolAdmin each see published A, B, and C (9 in the reported case: 7 AES + 2 Portal). Creator and school do not hide published school-type quizzes. Those roles may assign a catalog quiz to their own students/children. Students still only see assigned ∪ AudienceScope=Public. Unsubmitted Draft remains owner-only.",
  ],
  [
    "QZ-21",
  ...
  [
    "QZ-31",
    "Already-assigned students cannot be assigned again",
    "Student is already on quiz 36. Teacher or parent opens Assign and selects that student (or a mix that includes them).",
    "The picker shows Already assigned and the checkbox is locked. Assign skips existing rows. If every selected student already has a row, the API returns that they already have assignments. Existing QuizAttempt rows stay unchanged.",
  ],
  [
    "QZ-32",
    "No retry after the first attempt",
    "Student submitted quiz 36. Teacher or parent opens Assigned people. Student opens the quiz while the window is still open.",
    "Allow retry is not shown. The student cannot start a second attempt. View result stays hidden until the due date.",
  ],
];
...
const checklist = [
  ...
  "SchoolAdmin-created quizzes skip SchoolApproved: submit → PortalAdmin Approve (Pending → Approved) → PortalAdmin Publish.",
  "CampusAdmin-created quizzes: submit → SchoolAdmin School approve (Pending → SchoolApproved) → PortalAdmin Approve → PortalAdmin Publish.",
  ...
  "SchoolAdmin created: PortalAdmin only (Pending → Approved). CampusAdmin created: SchoolAdmin school-approves in the same school; other CampusAdmins cannot endorse it.",
  "After reject: only the quiz creator sees Resubmit for approval. PortalAdmin who rejected the quiz does not get that button and cannot resubmit.",
  "school-type: Parent may create; PortalAdmin approve + publish before assign.",
  "Unpublished (Draft lifecycle): unsubmitted WIP is owner-only. After Submit for approval, PortalAdmin sees the pipeline draft; SchoolAdmin sees Teacher/Coordinator/CampusAdmin submitted quizzes in the same school; CampusAdmin sees Teacher/Coordinator submitted quizzes in the same campus. Open /quizzes/:id and School approve / Reject.",
  "Published school-type quizzes: shared catalog — Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent can list and open them, regardless of creator or school. Students still need assignment or AudienceScope=Public.",
  "Teacher/Coordinator/Parent/SchoolAdmin/CampusAdmin list shows all published school-type quizzes (AES + ISL + Portal-created). Teacher/Coordinator/Parent may assign a catalog quiz to their own roster/children. SchoolAdmin/CampusAdmin assign stays org-scoped. Edit/archive stay owner/PortalAdmin scoped.",
  ...
  "Create, edit, settings, and assign do not collect Allowed Attempts. Each quiz allows one attempt. Retry and reassign of the same quiz are not supported.",
  "Assign skips students who already have a QuizAssignment row. Cancel of future unused assignments deletes that row so the student can be assigned later.",
  ...
  "CampusAdmin may create school-type quizzes (submit → SchoolAdmin then PortalAdmin), school-approve/reject campus Pending teacher/coordinator quizzes, list the shared published catalog, and open /quizzes manage routes.",
  "SchoolAdmin, CampusAdmin, and PortalAdmin may create school-type quizzes; SchoolAdmin/CampusAdmin submit → PortalAdmin approve (not school-approve). Teacher/Coordinator: School/Campus approve → SchoolApproved; Portal approve → Approved.",
...
Pending Approval ── not assignable; owner may edit until school/portal approval
     ...
     └── Parent (school types) → Portal Admin approve + publish</pre>
  ...
  <p><em>Teacher and Coordinator</em> share the same campus-scoped author workflow (school quiz types, submit for approval, assign/monitor in campus). They also see and may assign from the shared published school-type catalog (any school/creator) to their own roster. See §6b for Coordinator specifics.</p>
  <p><em>*CampusAdmin:</em> may create school-type quizzes (Practice, Assessment, Competition, Surprise; starts Pending; submit → SchoolAdmin, then PortalAdmin). May approve/reject Pending teacher/coordinator quizzes in campus (→ SchoolApproved, does not publish), list the shared published school-type catalog (any school/creator), and open /quizzes manage routes. Cannot publish. Assign stays campus-scoped.</p>
  <p><em>*Admin create:</em> SchoolAdmin, CampusAdmin, and PortalAdmin may create school-type quizzes (starts Pending). CampusAdmin submit is reviewed by SchoolAdmin (Pending → SchoolApproved) then PortalAdmin. SchoolAdmin submit is reviewed by PortalAdmin only (Pending → Approved). Coordinator creates school quiz types like Teacher. School/Campus admins may school-approve Teacher/Coordinator quizzes in scope; SchoolAdmin may also school-approve CampusAdmin quizzes in the same school; only PortalAdmin publishes.</p>
  <div class="note"><strong>Ownership / scope:</strong> Creator cannot self-approve. Unsubmitted <strong>Draft</strong> is owner-only. After Submit for approval, PortalAdmin may see the pipeline draft; SchoolAdmin may review Teacher/Coordinator submitted quizzes in the same school; CampusAdmin those in the same campus (School approve / Reject on <code>/quizzes/:id</code>). <strong>Published</strong> school-type quizzes are visible to Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent. Teacher/Coordinator/Parent may assign a catalog quiz to their own students/children. SchoolAdmin/CampusAdmin assign stays org-scoped. Edit/archive stay owner/PortalAdmin scoped.</div>

  <h2>6b. Coordinator role (quiz module)</h2>
  ${htmlList(coordinatorRoleSummary)}

  <h2>6a. Published quiz catalog visibility (GET /api/quizzes)</h2>
  ...
  ${htmlList([
    ...
    "EndAt > StartAt. Quiz and assign forms do not collect Allowed Attempts — each quiz allows one attempt. Assignment quota is always 1.",
    "Students who already have a QuizAssignment row are locked in the assign picker and skipped by the API. There is no reassign of the same quiz.",
    "Cancel: hard-delete future unused assignments only; restore lifecycle Assigned or Published (never Cancelled). After cancel, that student may be assigned again as a new row.",
  ...
  ${htmlList([
    "Shared catalog (Teacher/Coordinator/Parent/SchoolAdmin/CampusAdmin/PortalAdmin): see §6a — all published school-type quizzes, any school/creator.",
    "Parent list = shared published catalog ∪ assignments of linked children ∪ quizzes they created.",
    "Teacher/Coordinator list = shared published catalog ∪ assignments of roster students ∪ quizzes they created.",
    "Assignment board / per-quiz assignment list / monitoring / pending reviews: Parent = linked children; Teacher/Coordinator = roster; CampusAdmin = campus students; SchoolAdmin = school students; PortalAdmin = all. Rows the caller assigned remain visible even if a student later leaves scope.",
    "Parent may review/finalize attempts of linked children on viewable quizzes, and may view linked-child results.",
    "Teacher/Coordinator may review/finalize and monitor roster students on catalog quizzes they did not create. Cancel-upcoming still requires quiz ownership (or school/campus/portal manage scope).",
    ...
    "Student/parent results: auto-graded questions announce when the due date ends; teacher-review questions and full Completed wait until the owner clicks Completed. Show resultAnnouncedPercent on list, detail, dashboard, and result screens.",
  ...
  ${htmlList([
    ...
    "Quiz AllowedAttempts is always 1 on create/update/duplicate. Assignment and audience quotas are clamped to 1. There is no Allow retry or reassign.",
  ...
  <h2>15. UI routes</h2>
  ${htmlList([
    "/quizzes — manage catalog; New, Assignments, Pending reviews. Portal/School/Campus Admin: Edit requests tile filters the queued grant requests. Parent create school-type. Marks column shows obtained/total when a result score is available, otherwise total marks. No School column / school name on the list.",
    ...
    "/parent/quiz-dashboard, children history/result — parent flows. /parent/children/:id/history has no Attempts column.",
    ...
    "Mobile /quizzes (Teacher/Coordinator/Parent) — create (Parent → school-type), manage, submit for approval, assign (campus or linked audiences), duplicate/cancel, pending reviews, mark + finalize. Archive of Published/Assigned is PortalAdmin only (hidden for other roles).",
...
const docChildren = [
  ...
  docParagraph(
    "Teacher and Coordinator share the same campus-scoped author workflow (school quiz types, submit for approval, assign/monitor in campus). They also see and may assign from the shared published school-type catalog to their own roster. See §6b for Coordinator specifics.",
  ),
  docParagraph(
    "CampusAdmin: may create school-type quizzes (starts Pending; submit → SchoolAdmin then PortalAdmin). Approve/reject Pending teacher/coordinator quizzes in campus (→ SchoolApproved, does not publish), list the shared published school-type catalog (any school/creator), open /quizzes manage routes. Cannot publish. Cannot approve SchoolAdmin-created or other CampusAdmin-created quizzes. Assign stays campus-scoped. Coordinator creates school quiz types like Teacher.",
  ),
  docParagraph(
    "Ownership / scope: Creator cannot self-approve. Unsubmitted Draft is owner-only. After Submit for approval, PortalAdmin may see the pipeline draft; SchoolAdmin reviews Teacher/Coordinator submitted quizzes in the same school; CampusAdmin those in the same campus. Published school-type quizzes are a shared catalog for Teacher, Coordinator, SchoolAdmin, CampusAdmin, PortalAdmin, and Parent (any school, any creator). Teacher/Coordinator/Parent may assign a catalog quiz to their own students/children. SchoolAdmin/CampusAdmin assign stays org-scoped. Edit/archive stay owner/PortalAdmin scoped.",
    { run: { bold: true, color: "92400E" } },
  ),
  ...
  docHeading("6a. Published quiz catalog visibility", HeadingLevel.HEADING_2),
  ...
  docParagraph(
    "A quiz may be assigned to: One student, Group, Parent linked child or student, Class, Section, School, Multiple schools, or Public. school-type: Parent creators; assign to linked children only.",
  ),
  ...
  ...[
    ...
    "EndAt > StartAt. Assignment AllowedAttempts is always 1 (no form field). Existing students are skipped; the picker locks them.",
    "Cancel removes future unused assignments only; restore Assigned or Published.",
  ].map(docBullet),
  ...
  ...[
    "Shared catalog: see §6a — Teacher, Coordinator, Parent, and org admins see all published school-type quizzes.",
  ...
  ...[
    "/quizzes manage routes for Teacher/Coordinator/Parent.",
    "/quizzes manage detail: authors edit/submit/assign; SchoolAdmin/CampusAdmin/PortalAdmin approve or reject pending quizzes in review mode (Approve + Reject only). Open from the Draft tile — no /admin/quiz-approvals page.",
    "/quizzes/:id/assigned lists assigned students/children (Student, Window, Result, Actions — no Attempts column).",
    ...
    "Mobile /quizzes (Teacher/Coordinator/Parent): create, manage, submit for approval, campus or linked assign modes, duplicate/cancel, pending reviews, mark + finalize. Archive of Published/Assigned is PortalAdmin only.",
