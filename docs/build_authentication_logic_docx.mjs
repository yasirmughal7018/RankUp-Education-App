  ...
  sections: [
    {
      children: [
        ...
        p("API shared; React full; Flutter Mobile partial. Version: 23 Aug 2026."),
        p(
        ...
        bullet("Seven roles. Exclusive: Student, PortalAdmin, SchoolAdmin, CampusAdmin. Combinable: Teacher + Parent + Coordinator (any subset). Former Tutor accounts are remapped to Parent."),
        ...
        bullet("Parent companion grants/removes on Teachers/Coordinators are PortalAdmin-only. School/Campus may manage Teacher↔Coordinator in scope."),

        h2("2. Username & login"),
        bullet("Self-register and directory create: email required; username = normalized email; no CNIC rewrite on activate."),
        bullet("Mobile optional. Student roll required only when school selected. Parent never uses school/campus/roll."),
        ...
        bullet("Allowed: any subset of Teacher+Parent+Coordinator."),
        bullet("Blocked: Student/PortalAdmin/SchoolAdmin/CampusAdmin with any other role."),
        bullet("Parent profile: app_user_parents; student links: parent_student_relations (not class roster)."),
        bullet("Remove APIs: DELETE /api/directory/…/{id}/roles/{role} — companion only; not list primary; not sole role; Parent remove PortalAdmin-only on Teachers/Coordinators."),
        bullet("Self-service: POST /api/auth/me/role-requests (Parent/Teacher/Coordinator); DELETE /api/auth/me/roles/{role}."),
        ...
        bullet("Request: Teacher / Student / CampusAdmin / Coordinator. Parent cannot."),
        ...
        bullet("Directory grant/remove companions on teachers|parents|coordinators"),
        ...
        bullet("app_approval"),
        bullet("app_user_parents, parent_student_relations (legacy tutor tables remapped on startup)"),
