...
def build_doc():
    ...
    for item in [
        ...
        "Combinable on one login: Teacher + Parent + Coordinator (any subset; e.g. Parent+Teacher, Teacher+Parent, all four).",
        "Users may request Parent/Teacher/Coordinator as an extra role (app_user_role_request; account stays active) or "
        "self-remove Parent/Teacher/Coordinator when another role remains. Parents directory + Parent/Parent companions "
        "on Teachers/Coordinators are PortalAdmin-only; School/Campus may manage Teacher↔Coordinator in scope (see §7b).",
        "School/campus change (Teacher/Student/CampusAdmin/Coordinator) locks until destination apply or reject. Parent cannot request school/campus change. Student reject may optionally leaveWithoutSchool.",
    ...
    for item in [
        ...
        "Directory create (Student / Teacher / Parent / SchoolAdmin / CampusAdmin): email is required; username = email. Former Tutor accounts are remapped to Parent.",
    ...
    add_bullet(doc, "Examples allowed: Teacher+Parent; Teacher+Parent; Parent; Teacher+Parent+Coordinator. Blocked: SchoolAdmin+Teacher; CampusAdmin+Parent; Student+anything.")
    for item in [
        ...
        "DELETE /api/auth/me/roles/{role} removes Parent/Teacher/Coordinator and re-issues tokens; blocked if student groups reference that user+role; Tutor also blocked while parent_student_relations remain.",
    ...
    doc.add_heading("7b.1 Self-service request (Parent, Teacher, Coordinator)", level=2)
    for item in [
        "POST /api/auth/me/role-requests — request Parent, Teacher, Coordinator as additional role (does not lock account).",
    ...
    for item in [
        ...
        "SchoolAdmin/CampusAdmin must NOT grant or remove Parent on Teachers/Coordinators.",
    ...
    for item in [
        ...
        add_bullet(doc, item)
    doc.add_heading("7b.5 Parents directory (PortalAdmin)", level=2)
    for item in [
        "Web: /admin/directory/parents — PortalAdmin only (create, activate/deactivate, link/unlink students by CNIC or username).",
        "Profile table app_user_parents; links parent_student_relations (not class roster). Lookup id 2017.",
    ...
    for item in [
        "Who may request: Teacher, Student, CampusAdmin, Coordinator. Cannot: PortalAdmin, SchoolAdmin, Parent.",
    ...
    add_table(
        ...
        [
            ...
            ["app_user_role_request", "Pending self-service Parent/Teacher/Coordinator role requests"],
            ...
            ["app_user_parents", "Tutor slim profile"],
            ["parent_student_relations", "Tutor↔Student tuition links"],
        ],
        [3600, 5760],
    )
    ...
    add_table(
        ...
        [
            ...
            ["app_user_parents", "mobile", "cnic (no school/campus)"],
        ],
        [2400, 2800, 4160],
    )
    ...
    add_table(
        ...
        [
            ...
            ["POST", "/api/auth/me/role-requests", "JWT", "Request additional Parent/Teacher/Coordinator"],
            ...
            ["POST", "/api/directory/teachers/{id}/roles/parent", "PortalAdmin", "Grant Tutor to Teacher"],
            ...
            ["POST", "/api/directory/parents/{id}/roles/parent", "PortalAdmin", "Grant Tutor to Parent"],
            ["POST", "/api/directory/coordinators/{id}/roles/parent", "PortalAdmin", "Grant Parent to Coordinator"],
            ["POST", "/api/directory/coordinators/{id}/roles/parent", "PortalAdmin", "Grant Tutor to Coordinator"],
            ...
            ["POST/DELETE", "/api/directory/tutors/{id}/students…", "PortalAdmin", "Link/unlink parent–student"],
    ...
    add_note(
        doc,
        "Version",
        "Aligned with live API (16 Aug 2026). Combinable: Teacher+Parent+Coordinator. "
        "Exclusive: Student, PortalAdmin, SchoolAdmin, CampusAdmin. Parents directory + Parent/Tutor "
        ...
        "Flutter Mobile = auth + school-change request + role switch learner surfaces as implemented; "
        "directory grant/removes remain Web-only.",
        "EEF8F1",
    )

    doc.core_properties.title = "RankUp Education — Authentication & Login Logic"
    doc.core_properties.subject = (
        "Authentication role, email username, scoped activation, forgot-password first-wins, "
