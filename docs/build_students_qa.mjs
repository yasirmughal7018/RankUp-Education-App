...
const DOC_DATE = "11 Aug 2026";
const DOC_TITLE = "RankUp Education — Students Business & QA Guide";
const DOC_SUBTITLE =
  "Student identity, registration, directory, quizzes, rankings, dashboards, school change, and known gaps — as implemented.";

const identityRules = [
  ["Role exclusivity", "Student cannot combine with any other role (UserRoleRules). Adding Student when another role exists, or adding any role when Student exists, fails."],
...
const registrationRules = [
  ...
  ["Parent/Teacher", "Grade/section rejected if sent for Parent or Teacher"],
];
...
const approvalActivation = [
  ["PortalAdmin", "Can approve and activate any registration"],
  ["SchoolAdmin", "Can activate Student/Teacher with school in scope; cannot activate no-school / Parent-only"],
...
const directoryRules = [
  ["GET /directory/students", "Filters: schoolId, campusId, grade, search, paging. Scope by admin school/campus. Search name/username/roll."],
  ...
  ["PUT /directory/students/{id}", "Update name, campus (same school), roll, grade, section."],
...
const checklist = [
  ...
  "Roll required only when school is selected; Parent never gets grade/section/school.",
  ...
  "Student school-change fully locks the account.",
...
<main>
  ...
  <div class="ok"><strong>Canonical Student MVP:</strong> exclusive Student role; register/directory with real grade (Class lookup) + section; take assigned quizzes (incl. offline); self history; class/school peer rankings from best attempt %; quiz-only dashboards. Parent linking is admin/parent-side only.</div>
...
const docChildren = [
  ...
  docParagraph(
    "Canonical Student MVP: exclusive role; register/directory with Class grade + section; assigned quizzes + offline; self history; class/school peer rankings; honest quiz-only dashboards.",
