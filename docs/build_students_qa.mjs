...
const identityRules = [
  ...
  ["Class history table", "app_user_student_class_history: one open row (ended_at null) = current placement; closed rows keep prior grade/section periods. Written on directory create/update and registration activate; existing students backfilled on schema init."],
  ...
  ["Self class history", "GET /students/me/class-history — Student only; newest first. Shown read-only on web /account."],
];
...
const approvalActivation = [
  ...
  ["On Student activate", "CreateProfileForRoleAsync uses registration_grade + registration_section — no hardcoded grade 1 / section A; also opens class-history RegistrationActivate row"],
...
const directoryRules = [
  ...
  ["Web filters UI", "PortalAdmin: School + Campus + Grade (Class lookup dropdown) + status. SchoolAdmin: Campus + Grade + status (school locked). CampusAdmin: Grade + status only (school/campus locked; no School/Campus dropdowns)."],
  ["Create/Edit student dialog", "CampusAdmin: School and Campus fields hidden; placement = signed-in school/campus. SchoolAdmin: School hidden, Campus selectable. Grade is Class lookup dropdown for all roles."],
  ...
  ["POST /directory/students", "Requires FullName, email/username, RollNumber, Grade (>0), Section, SchoolId, CampusId. Creates active Student ready for password setup. Does not re-validate Class lookup (unlike register). Opens class-history DirectoryCreate row."],
  ["PUT /directory/students/{id}", "Update name, campus (same school), roll, grade, section (and school/campus where allowed). Grade/section change closes prior class-history row and opens a new one."],
...
const webRoutes = [
  ...
  ["/account", "Student: Class history section (GET /students/me/class-history) — prior + current grade/section periods"],
...
const scenarios = [
  ...
  ["STU-06", "Directory create", "Admin creates student with grade+section+roll+school+campus.", "Active Student provisioned; ready for set-initial-password; class-history open row created."],
  ["STU-06b", "Class history on promote", "Directory updates student from grade 1 to grade 4.", "Prior row ended_at set; new open row for grade 4; Account /account lists both."],
...
const checklist = [
  ...
  "Class history: create/activate opens a row; grade/section update closes and opens; /account shows GET /students/me/class-history.",
