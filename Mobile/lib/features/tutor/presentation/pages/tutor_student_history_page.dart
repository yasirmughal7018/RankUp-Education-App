...
/// Quiz history for one linked student (tutor’s own quizzes only).
class TutorStudentHistoryPage extends ConsumerWidget {
  const TutorStudentHistoryPage({required this.studentId, super.key});

  final int studentId;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final historyAsync = ref.watch(tutorStudentHistoryProvider(studentId));
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Refresh',
            onPressed: () =>
                ref.invalidate(tutorStudentHistoryProvider(studentId)),
      ...
      body: historyAsync.when(
        ...
        data: (history) {
          if (history.items.isEmpty) {
            return const Padding(
              ...
              child: AppEmptyState(
                ...
                message:
                    'Attempts on quizzes you created for this student will appear here.',
              ),
            );
          }
          ...
          return RefreshIndicator(
            onRefresh: () async {
              ref.invalidate(tutorStudentHistoryProvider(studentId));
              await ref.read(tutorStudentHistoryProvider(studentId).future);
            },
